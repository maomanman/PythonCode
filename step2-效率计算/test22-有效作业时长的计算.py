...
def batchGetYangTime():
    ...
    test22_result = pda.DataFrame(columns={'文件名称', '采样时间'})
    i = 0
    for fName in fileIndexData.文件名称:
        fData = pda.read_excel(r'D:\mmm\轨迹数据集\汇总/' + fName)
        yangT = selfShape.calYangTime(fData)
        test22_result.loc[i] = [fName, yangT]
        i = i + 1
        if i % 10 == 0:
            test22_result.to_excel(rootPath + '/test23_采样时间.xlsx')
...
def batchGetWorkTime():
    ...
    test22_ParaTimeList = pda.read_excel(rootPath + '/test23-中期汇总_总时间统计.xlsx')
    ...
    for ind in fileIndexData.index:
        ...
        if fileName == test22_ParaTimeList.loc[ind, '文件名称']:
            ...
            test22_return_info.loc[ind] = [fileName, totalTime, str(workTime),
                                           pda.to_timedelta(totalTime, unit='s').total_seconds(),
                                           workTime.total_seconds()]

        if ind % 10 == 0:
            test22_return_info.to_excel(rootPath + '/test23_有效工作时长.xlsx')
