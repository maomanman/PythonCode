...
def SplitFiledByMoreAtrributes(data, fileName):
    """
    联合多种属性分割地块
    ...
    if dict['velocity'] not in data.columns:  # 标签没有则加入新标签
        ...
        # region 存在野点才需要另外存储和删除
        if len(newDriftIndex) > 0:
            # 第四类野点数据进行另存为
    ...
    data['XX'] = 0
    data['YY'] = 0
    data.loc[0:len(x) - 1, 'XX'] = x
    data.loc[0:len(x) - 1, 'YY'] = y
...
if __name__ == '__main__':
    ...
    # 处理excel数据
    data, x, y = SplitFiledByMoreAtrributes(data, fileName)
